...
import updateFruit from '../actions/fruitInvUpdate';
import {bindActionCreators} from 'redux';

class FruitDept extends Component{
    ...
    increment =(operation, index)=>{
        ...
        this.props.updateFruit(operation,index);

     }

    render(){
        const fruitInventory = this.props.fruitData.map((item,i)=>{
            return (
                <div key={i}>
        ...
        return(
            <div>
                <h1> The fruit food department</h1>
                <ul>
                    {fruitInventory}
                </ul>
            </div>
...
export default connect(mapStateToProps,mapDispatchToProps)(FruitDept);